...
class TestMrpPlm(TestPlmCommon):
    ...
    def test_multiple_bom_lines_with_same_product(self):
        """
        Test eco when there are multiple BoM lines for the same product.
        """
        self.env['mrp.bom.line'].create({'product_id': self.table_leg.id, 'bom_id': self.bom_table.id, 'product_qty': 5})
        mrp_eco = self._create_eco('mrp eco', self.bom_table, self.eco_type.id, self.eco_stage.id)
        mrp_eco.action_new_revision()
        self.assertEqual(len(mrp_eco.bom_change_ids), 0)
