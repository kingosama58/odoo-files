...
class PosConfig(models.Model):
    ...
    @api.constrains("module_pos_discount", "module_loyalty")
    def _check_blackbox_config(self):
        for config in self:
            if config.iface_fiscal_data_module and config.module_pos_discount and config.module_loyalty:
    ...
    def _check_cash_rounding(self):
        ...
        if self.rounding_method.rounding != 0.05 or self.rounding_method.rounding_method != "HALF-UP":
            raise ValidationError(_("The rounding method must be set to 0.5 and “Nearest”"))
